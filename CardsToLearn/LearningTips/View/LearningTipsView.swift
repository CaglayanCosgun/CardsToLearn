...
struct LearningTipsView: View {
    ...
    @State var currentImage: Tips = sampleTips.first!
    ...
    func indexOf(image: Tips)-> Int{
        if let index = sampleTips.firstIndex(of: image){
            return index
        }
        return 0
    }
        ...
        VStack{
            ...
            ZStack(alignment: .leading){
                ...
                GeometryReader{
                    ...
                    Capsule()
                        .fill(.green)
                        .frame(width: CGFloat(indexOf(image: currentImage)) / CGFloat(sampleTips.count - 1) * size.width)
    ...
    @ViewBuilder
    func ImageView(image: Tips) -> some View {
        GeometryReader {
            let size = $0.size
